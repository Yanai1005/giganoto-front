...
export class MinigameManager {
  ...
  startContinuousRumble() {
    ...
    const low_amp = 0.1 + (baseRarity - 1) * 0.03; // 0.1 - 0.22 (大幅に削減)
    const high_amp = 0.15 + (baseRarity - 1) * 0.03; // 0.15 - 0.27 (大幅に削減)
  ...
  temporaryStrongRumble() {
    // 一時的に強い振動を発生させる（安全なレベルに調整）
    this.scene.jcHID.rumble(120, 240, 0, 0.25); // 0.6から0.25に大幅削減
