...
class VibrationHuntScene extends Phaser.Scene {
  ...
  createTitleScreen() {
    ...
    circle2.fillCircle(850, 500, 120);

    const circle3 = this.add.graphics();
    circle3.fillStyle(0x6a5a8a, 0.06);
    circle3.fillCircle(900, 150, 60);

    // 振動エフェクトアイコン
    this.createVibrationIcon(500, 150);

    // メインタイトル
    const mainTitle = this.add.text(500, 220, '強震動探し', {
      fontSize: '48px',
      fill: '#ffffff',
    ...
    // サブタイトル
    this.add.text(500, 280, '一番振動が強い場所を探そう！', {
      fontSize: '18px',
      fill: '#b8c6ff',
      fontFamily: 'Arial, sans-serif',
      fontStyle: 'italic'
    ...
    // ゲーム説明
    this.add.text(500, 320, 'Joy-Conの振動を頼りに隠された宝を見つけよう！', {
      fontSize: '16px',
      fill: '#b8c6ff',
      fontFamily: 'Arial, sans-serif'
    }).setOrigin(0.5);
    ...
    this.add.text(500, 350, 'Nintendo Switch 2のひみつ展のオマージュ', {
      fontSize: '12px',
      fill: '#8899bb',
      fontFamily: 'Arial, sans-serif',
      fontStyle: 'italic'
    ...
    // スタートボタン
    const startButton = this.createStylishButton(500, 420, 'ゲームスタート', '#00d4aa', '#00b899');
    ...
    // 操作説明
    this.add.text(500, 560, 'マウスで操作、クリックで決定', {
      fontSize: '14px',
      fill: '#6677aa',
      fontFamily: 'Arial, sans-serif'
    }).setOrigin(0.5);
  ...
  transitionToConnection() {
    ...
    fadeGraphics.fillRect(0, 0, 1000, 600);
  ...
  createJoyConConnectionUI() {
    ...
    const circle2 = this.add.graphics();
    circle2.fillStyle(0x5a4a7a, 0.08);
    circle2.fillCircle(850, 480, 120);

    const circle3 = this.add.graphics();
    circle3.fillStyle(0x6a5a8a, 0.06);
    circle3.fillCircle(900, 120, 60);

    // Joy-Conアイコンの作成（シンプルな図形で表現）
    this.joyconIcon = this.createJoyConIcon(500, 100);

    // メインタイトル（グロー効果付き）
    this.titleText = this.add.text(500, 160, 'Joy-Con接続', {
      fontSize: '32px',
      fill: '#ffffff',
      fontFamily: 'Arial, sans-serif',
      fontWeight: 'bold',
    ...
    // スタイリッシュな接続ボタン
    this.connectButton = this.createStylishButton(500, 440, 'Joy-Con接続', '#00d4aa', '#00b899');
    ...
    // 戻るボタン
    this.backButton = this.createStylishButton(500, 500, 'タイトルに戻る', '#4a9eff', '#3a8eef');
    ...
    if (!('hid' in navigator)) {
      ...
      warningBox.fillRoundedRect(180, 520, 640, 50, 10);
      warningBox.lineStyle(2, 0xff6666, 1);
      warningBox.strokeRoundedRect(180, 520, 640, 50, 10);

      this.add.text(500, 545, '⚠️ このブラウザはWebHID APIに対応していません', {
        fontSize: '16px',
        fill: '#ff8888',
        fontFamily: 'Arial, sans-serif'
      }).setOrigin(0.5);
    ...
    // 操作説明（画面下部）
    this.add.text(500, 580, 'マウスで操作、クリックで決定', {
      fontSize: '12px',
      fill: '#8899bb',
      fontFamily: 'Arial, sans-serif',
      fontStyle: 'italic'
